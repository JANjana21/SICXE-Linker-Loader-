...
while(flag == 0):
    ...
    Address_entry=None
    labelFrame = LabelFrame(win,text = "Choose " , padx=50,pady=50)
    labelFrame.pack()
    Label(labelFrame,text="SIC OR SICXE").pack()
    ...
    if(choice =="SIC" or choice =="sic"):
        ...
        while (j <len(data)):
            ...
            curr_add=address[j][2:] #ADDRESS HAS ALL THE STARTING ADDRESSES FOR EACH T RECORD HERE WE ARE ACCESSING EACH ONE OF THEM AND REMOVING 0x
            ...
            while(curr_add<last_add):   
                col2 = add[:3]+'0'  #HERE WE ARE TAKING THE FIRST 3 DIGITS OF THE ADDRESS AND ADDING A ZERO TO IT FOR EXAMPLE IF THE  ADDRESS = 1075 THIS MAKES IT 1070 
                row2 = add[3:]      #THIS TAKES THE LAST DIDGIT OF THE ADDRESS SO WE CAN MATCH IT TO THE ROW LETS SAY ADDRESSS =1075 THEN THIS =5 WHICH MEANS WE WILL ADD IT TO COL=5
...
#------------------------------------------------------------- GETTING THE ESTABLE ------------------------------------------------------------------------------------
        ...
        # EXT_SYM.write("Control Section  Address Length Symbol_Name \n")
        # for names in H_name:
        #     EXT_SYM.write(names+"\n")
        # df.to_csv(r'C:\Users\janad\Desktop\Project\out.txt' ,sep=' ', mode='a')
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------
...
#----------------------------------------------- UPLAODING THE MEMORY ----------------------------------------------------------------------
        ...
        while (hh <len(data)):
            ...
            curr_add=T_new_start_add[hh][2:] #ADDRESS HAS ALL THE STARTING ADDRESSES FOR EACH T RECORD HERE WE ARE ACCESSING EACH ONE OF THEM AND REMOVING 0x
...
#-----------------------------------------------------------------------------------------------------------------------
        ...
        for line2 in FILEIN:
            if(line2[0]=='H'):
                ...
                mod_name = df.loc[name2]["Symbol_Name"][0]
                mod_name2 = df.loc[name2]["Symbol_Name"][1]
                endx = mod_name2[5:]
                listx = mod_name[5:]
                label_address.append(endx.replace(':',''))
                label_address.append(listx.replace(':',''))
                sym_add = mod_name[:5]
                sym_add2 = mod_name2[:5]
                labels.append(sym_add2.replace(':',''))
                labels.append(sym_add.replace(':',''))
                # print(start2)
